...
public class MapController {
    ...
    public SseEmitter sub(@PathVariable Long userId,
                          @RequestParam List<Long> restaurantId,
                          @RequestParam List<String> x, @RequestParam List<String> y,
                          @RequestParam List<String> name, @RequestParam List<String> placeUrl) {
        return mapService.addEmitter(userId, x, y, restaurantId, name, placeUrl);
    }

    @PostMapping("/disconnect/{userId}")
    public ResponseEntity<String> disconnect(@PathVariable Long userId) {
        mapService.removeSSE(userId);
        return ResponseEntity.ok("SSE 연결 종료 성공");
    }
